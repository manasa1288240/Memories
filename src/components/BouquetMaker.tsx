...
interface BouquetMakerProps {
  ...
  readOnly?: boolean;
  /** Full Tailwind class string for themed buttons, e.g. "bg-sky-500 hover:bg-sky-600 text-white" */
  buttonClass?: string;
}

const DEFAULT_BUTTON = "bg-pink-600 hover:bg-pink-700 active:bg-pink-800 text-white shadow-md shadow-pink-200";

const FLOWERS = [
  { name: "Blush Rose 🌹", label: "Blush Rose" },
  { name: "Pink Tulip 🌷", label: "Pink Tulip" },
  { name: "Peony Blossom 🌸", label: "Peony Blossom" },
  { name: "Rustic Carnation 🌺", label: "Rustic Carnation" },
];
...
export default function BouquetMaker({
  ...
  const handleAddFlower = (flowerName: string) => {
    if (selectedFlowers.length >= 10) return;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedFlowers.length === 0) { alert("Please add at least one flower!"); return; }
    if (!creator.trim()) { alert("Please specify your name so they know who gifted it!"); return; }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {bouquets.map((b) => (
              <div key={b.id} className="bg-white p-6 rounded-3xl border border-pink-100 shadow-md relative overflow-hidden flex flex-col justify-between hover:shadow-lg transition-shadow">
                <div className="relative h-32 flex items-center justify-center bg-pink-50/50 rounded-2xl mb-4 p-2 overflow-hidden">
                  <div className={`absolute bottom-0 w-24 h-16 z-10 origin-bottom rounded-b-2xl transition-all duration-300 ${
                    ...
                    : b.wrap === "pink-lace" ? "bg-pink-100/90 border-2 border-pink-300/40 rotate-[10deg] rounded-t-md"
                    : "bg-white border-2 border-slate-100 rotate-[-5deg] rounded-t-md"
                  }`} />
                  {b.ribbon !== "none" && (
                    <div className={`absolute bottom-3 w-8 h-8 rounded-full flex items-center justify-center z-20 text-[10px] ${
                      b.ribbon === "pink" ? "bg-pink-400 text-white shadow-sm"
                      : b.ribbon === "red" ? "bg-red-500 text-white shadow-sm"
                  ...
                  {b.flowers.map((name, idx) => {
                    ...
                    return (
                      <div key={idx} style={{ transform: `translate(${Math.sin((angle * Math.PI) / 180) * distance}px, ${-Math.cos((angle * Math.PI) / 180) * distance - 10}px) rotate(${angle}deg)` }} className="absolute text-3xl z-5 select-none">
                        {emoji}
                      </div>
                    );
                  })}
                ...
                <div className="flex-grow flex flex-col justify-between">
                  <div>
                    ...
                    <p className="font-serif text-pink-700 text-lg leading-snug mt-2 text-center pb-2">"{b.message}"</p>
                  </div>
                  <div className="pt-2 border-t border-pink-50 text-right mt-2">
                    <p className="text-[10px] font-mono text-pink-400">Gifted with love from <span className="font-bold text-pink-600">{b.creator}</span></p>
                  </div>
                </div>
              </div>
            ))}
    ...
    <div id="bouquet-maker" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-7 bg-white p-6 rounded-2xl border border-pink-100 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-xs font-mono font-bold text-pink-600 uppercase mb-2">Step 1: Pick Flowers (Max 10)</label>
            ...
            <div className="bg-pink-50/30 p-3 rounded-xl border border-dashed border-pink-200 min-h-[50px] flex flex-wrap gap-2 items-center">
              {selectedFlowers.length === 0 ? (
                <p className="text-xs italic text-pink-400 mx-auto">No flowers in your bouquet yet. Click above to add!</p>
              ) : (
                selectedFlowers.map((name, idx) => (
                  ...
                  >
                    <span>{name}</span>
                    <button type="button" onClick={() => handleRemoveFlowerOfIndex(idx)} className="text-pink-300 hover:text-pink-600 duration-150 ml-1 font-bold text-xs">×</button>
                  </motion.span>
                ))
              )}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-mono font-bold text-pink-600 uppercase mb-2">Step 2: Paper Wrapping Style</label>
              <select id="select-wrap-style" value={selectedWrap} onChange={(e) => setSelectedWrap(e.target.value as any)} className="w-full text-sm bg-pink-50/35 border border-pink-200 rounded-lg p-2.5 text-pink-900 focus:ring-1 focus:ring-pink-300 outline-none">
                {WRAPS.map((wrap) => <option key={wrap.id} value={wrap.id}>{wrap.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-mono font-bold text-pink-600 uppercase mb-2">Step 3: Satin Ribbon</label>
              <select id="select-ribbon-style" value={selectedRibbon} onChange={(e) => setSelectedRibbon(e.target.value as any)} className="w-full text-sm bg-pink-50/35 border border-pink-200 rounded-lg p-2.5 text-pink-900 focus:ring-1 focus:ring-pink-300 outline-none">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            <div className="md:col-span-8">
              <label className="block text-xs font-mono font-bold text-pink-600 uppercase mb-1">Step 4: Attached Flower Greeting Card</label>
              <input id="input-bouquet-card-msg" type="text" placeholder="Wishing you a beautiful year packed of blossoms!..." value={message} onChange={(e) => setMessage(e.target.value)} className="w-full text-sm bg-pink-50/35 border border-pink-200 rounded-lg p-2.5 text-pink-900 focus:ring-1 focus:ring-pink-300 outline-none" />
            </div>
            <div className="md:col-span-4">
              <label className="block text-xs font-mono font-bold text-pink-600 uppercase mb-1">Your Name</label>
              <input id="input-bouquet-creator" type="text" placeholder="Aunt Sarah" value={creator} onChange={(e) => setCreator(e.target.value)} required className="w-full text-sm bg-pink-50/35 border border-pink-200 rounded-lg p-2.5 text-pink-900 focus:ring-1 focus:ring-pink-300 outline-none" />
            </div>
          </div>

          <div className="flex justify-end gap-3 pt-2">
            <button id="btn-clear-bouquet" type="button" onClick={clearCurrentBouquet} className="text-xs font-medium text-pink-500 hover:text-pink-700 px-4 py-2 cursor-pointer">
              Reset Flower Box
            </button>
            <button
              id="submit-bouquet-btn"
              type="submit"
              className={`px-6 py-2.5 ${buttonClass} font-medium text-sm rounded-xl flex items-center gap-2 cursor-pointer`}
            >
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
        <div className="bg-pink-50/70 border-2 border-dashed border-pink-200 p-6 rounded-2xl flex flex-col items-center justify-between min-h-[350px] relative overflow-hidden shadow-xs">
          <div className="absolute top-2 right-2 px-2 py-0.5 bg-pink-100 text-[9px] font-mono font-bold text-pink-500 rounded uppercase tracking-wider select-none">Live Preview Box</div>
          ...
          <div className="flex-grow flex items-center justify-center relative w-full h-[220px]">
            {selectedFlowers.length === 0 ? (
              ...
              <div className="relative w-48 h-48 flex items-center justify-center">
                <div className={`absolute bottom-0 w-36 h-28 z-10 origin-bottom rounded-b-3xl transition-all duration-300 ${
                  selectedWrap === "vintage" ? "bg-amber-100 border border-amber-300 rotate-[4deg] shadow-md bg-[radial-gradient(#b45309_0.5px,_transparent_0.5px)] bg-[size:10px_10px]"
                  : selectedWrap === "brown" ? "bg-amber-700/80 border border-amber-800 rotate-[-24deg] rounded-t-xl"
                  : selectedWrap === "pink-lace" ? "bg-pink-100/90 border-2 border-pink-300/40 rotate-[10deg] shadow-[0_0_12px_rgba(244,63,94,0.15)] rounded-t-lg"
                  : "bg-white border-2 border-slate-100 rotate-[-5deg] shadow-sm rounded-t-xl"
                }`} />
                {selectedRibbon !== "none" && (
                  <div className={`absolute bottom-6 w-12 h-12 rounded-full flex items-center justify-center z-20 ${
                    selectedRibbon === "pink" ? "bg-pink-400 text-white animate-pulse shadow-md"
                    : selectedRibbon === "red" ? "bg-red-500 text-white shadow-md"
                ...
                {selectedFlowers.map((name, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      animate={{ scale: 1, x: Math.sin((angle * Math.PI) / 180) * distance, y: -Math.cos((angle * Math.PI) / 180) * distance - 20, rotate: angle }}
          ...
          <div className="w-full text-center mt-3 pt-3 border-t border-pink-200/50">
            ...
            <p className="font-serif italic text-pink-700 text-xs px-2 line-clamp-2">"{message || "Happy celebration flowers! 💐"}"</p>
          </div>
        </div>
              ...
              {bouquets.map((b) => (
                <div key={b.id} className="text-xs bg-pink-50/40 p-2.5 rounded-lg border border-pink-100/50 flex justify-between items-start gap-2">
                  <div className="flex-grow">
                    <p className="font-semibold text-pink-900 leading-snug">Bouquet of {b.flowers.map(f => f.slice(0, 2)).join(", ")}</p>
                    <p className="text-[10px] italic text-pink-600 font-serif mt-1">"{b.message}"</p>
                    <p className="text-[9px] font-mono text-pink-400 mt-1">from <span className="font-bold text-pink-500">{b.creator}</span> • Wrapping: {b.wrap}</p>
                  </div>
                  {onDeleteBouquet && (
                    <button id={`delete-bouquet-${b.id}`} onClick={() => onDeleteBouquet(b.id)} className="text-pink-300 hover:text-red-500 cursor-pointer">
